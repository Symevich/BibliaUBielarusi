async function loadExhibit(section, id) {
        try {
          const response = await fetch("../json/catalog_be.json");
          ...
          if (!exhibit) {
            document.getElementById("container").innerHTML =
              "<p>Экспанат не знойдзены.</p>";
            return;
          }

          // генеруем кнопку "Назад"
          const backBtnHTML =
            section === "0" && String(id) === "0"
              ? `<a href="../index.html" class="back-btn">&lt; Выбар секцый</a>`
              : `<a href="./section.html?section=${section}" class="back-btn">&lt; Назад у секцыю</a>`;
          ...
          if (exhibit.img && exhibit.img.length && exhibit.img !== "[]") {
            ...
            if (cleanImgs.length > 1) {
                ...
                <div class="carousel">
                  <div class="carousel-track">
                    ${cleanImgs
                      .map(
                        (src) =>
                          `<div class="carousel-slide"><img src="${src}" alt="Фота экспаната"></div>`
                      )
                      .join("")}
                  </div>
              ...
              imgBlock = `<img src="${cleanImgs[0]}" alt="Фота экспаната" class="exb-img">`;
            }
          }
          ...
          document.getElementById("container").innerHTML =
            "<p>Памылка загрузкі экспаната.</p>";
          console.error(err);
        }
      }
      ...
      if (section && id) {
        ...
        document.getElementById("container").innerHTML =
          "<p>Не зададзены параметры section і id.</p>";
      }